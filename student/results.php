...
.path-student .container-fluid .jumbotron{
...
$pre_result="";
 $sql="SELECT *
FROM mdl_course_modules
WHERE  completionexpected >'".$at."'";



    $resactivity=$DB->get_records_sql($sql);


//var_dump($resactivity);


foreach($resactivity  as $act)
{
//var_dump($act);
$act_id=$act->id;
    $module=$act->module;
...
$quiz_submission=count($attempts);
                if (!empty($attempts))
                {
if(empty($pre_result))
{
...
WHERE `id` = '$instance'");
                        ...
                        $attempts=sizeof($attempts);
                           ...
                           $pre_result.= html_writer::end_tag('td');
                             $pre_result.= html_writer::end_tag('tr');
    printdataact($coursename,'Quiz',$activityname,$descact,$attempts,$grade);
...
//echo date('m/d/Y h:i:s', $check_latest_submitted_lab);
...
$submissions=sizeof($submissions);
                        printdataact($coursename,'Lab',$activityname,$descact,$submissions,$usergrade);
...
$pre_result.= html_writer::end_tag('table');
//echo $pre_result;
//echo $data;
print <<<END
...
if($counting==0){
echo '<h3>'."No completed tests today".'</h3>';
}
...
END;
 ...
 <div class="span2 ttbtn">
 <a class="te1 btn btn-primary btn-lg pull-right" value="submit">
 Submissions<br>
END;
    echo $no_of_submission;
 print <<<END
    </a><a class="btn btn-primary btn-lg pull-right" value="submit">
 Grade<br><span>
END;
    echo $act_grade;
 print<<<END
