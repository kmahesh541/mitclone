...
$PAGE->set_url("$CFG->httpswwwroot/teacher/index.php");
...
} else if (!empty($SESSION->wantsurl) && file_exists($CFG->dirroot.'/teacher/weblinkauth.php')) {
    // Handles the case of another Moodle site linking into a page on this site
    //TODO: move weblink into own auth plugin
    include($CFG->dirroot.'/teacher/weblinkauth.php');
        ...
        if (!empty($userauth->config->expiration) and $userauth->config->expiration == 1) {
            if ($userauth->can_change_password()) {
                ...
                if (!$passwordchangeurl) {
                    $passwordchangeurl = $CFG->httpswwwroot.'/teacher/change_password.php';
                }
            } else {
                $passwordchangeurl = $CFG->httpswwwroot.'/teacher/change_password.php';
            }
...
if (empty($SESSION->wantsurl)) {
    ...
    if ($referer &&
            $referer != $CFG->wwwroot &&
            $referer != $CFG->wwwroot . '/' &&
            $referer != $CFG->httpswwwroot . '/teacher/' &&
            strpos($referer, $CFG->httpswwwroot . '/teacher/?') !== 0 &&
            strpos($referer, $CFG->httpswwwroot . '/teacher/index.php') !== 0) { // There might be some extra params such as ?lang=.
...
if (!empty($SESSION->loginerrormsg)) {
    ...
    redirect(new moodle_url('/teacher/index.php'));
}
...
if (isloggedin() and !isguestuser()) {
    ...
    $logout = new single_button(new moodle_url($CFG->httpswwwroot.'/teacher/logout.php', array('sesskey'=>sesskey(),'loginpage'=>1)), get_string('logout'), 'post');
    $continue = new single_button(new moodle_url($CFG->httpswwwroot.'/teacher/index.php', array('cancel'=>1)), get_string('cancel'), 'get');
