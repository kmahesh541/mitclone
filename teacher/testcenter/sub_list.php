...
foreach ($all_data as $data) {
    ...
    $loginstatus=$DB->get_field('userinfo_tsl', 'loginstatus', array('userid' => $userinfo->id));
...
//Add span id to submission info
        ...
        if($grades!==get_string('nograde')){
            if($display_flag){if($loginstatus){$gradeCount++;}}
    ...
    if($display_flag){
        if($loginstatus) {
            ...
            echo '<tr>
				...
				<td><a target="_blank" href=" ' . $CFG->wwwroot . '/teacher/student_profile.php?sid=' . $userinfo->id . '">' . $rollno . '</a></td>
				<td><a target="_blank" href="' . $CFG->wwwroot . '/teacher/student_profile.php?sid=' . $userinfo->id . '">' . $userinfo->firstname . '</a></td>
				<td style="text-align:center">' . $section_name . '</td>
				<td>' . $subtime . '</td>
				<td style="text-align:center"><a href="' . $usersubcount . '" target="_blank">' . $nsub . '</a></td>
				<td style="text-align:center">' . $grade . '</td>
				<td>
				<span class="watchlist-status'.$userinfo->id.'" style="display:none">'.$watchliststatus.'</span>
				<img data-ref="' . $watchliststatus . '" id="' . $userinfo->id . '" class="watchlist" src=" ' . $CFG->wwwroot . '/teacher/testcenter/images/' . $watchlist_icon . '" width="  16px"/></td>
